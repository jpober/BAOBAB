...
yielding a $\sim2\%$ error on the $z\sim1$ distance scale, and, in turn, significant
...
\hline\hline
BAOBAB & 0.67 & 9.6\% & 18.6\% & 0.72 & 4.7\% \\
BAOBAB & 0.89 & 6.7\% & 18.4\% & 0.74 & 3.5\% \\
BAOBAB & 1.18 & 4.9\% & 17.9\% & 0.74 & 2.7\% \\
\hline
...
predicted) is to degrade the significance of our BAO detections in each of the redshift bins by $\sim 0.5\sigma$, our $H(z)$,
$D_A$, and $R$ constraints by $\sim1$, $\sim5$, and $\sim0.5-1$ percentage points, respectively.
...
our BAO detections in each band are increased by $\sim 0.5\sigma$, our $H(z)$, $D_A$, and $R$ constraints are improved
by $\sim0.5$, $\sim6$, and $\sim0.5$ percentage points, respectively.  From this analysis, it is clear
...
limited 132-tile BAOBAB observation yields a $z\sim1$ distance scale uncertainty of $\sim1.8\%$ (compared with $1.9\%$ for our
...
array yields a distance scale uncertainty of $2.2\%$.
...
ignoring all foreground contamination.  The result is that a 1-year observation
of three independent declination fields
\pvm{This is 1 year *each*, except really only 8 hours out of 180 days, i.e., 
8*180 hours, each? To me, it would be clearer to give the total number of hours
and square degrees in the various example cases, with only a global 
clarification of the relation of this to declinations and how long it would 
actually take to accumulate this many hours of observing, but I guess it is 
debatable. In any case, I would at least sprinkle a few more reminders of what 
the numbers you give really mean in there. }
yields a distance scale uncertainty of 1.4\%, an increase of half a percentage 
point over the same observation including foreground emission.  
\pvm{Should make clear that this is combined over redshifts (it is, right?)}
\pvm{When you have a change from like 2\% to 1.5\%, I think it sounds a lot 
better to call this a ``25\% improvement'', or something like that, than a 
``0.5 percentage point'' improvement (especially in other places where one
neither number involved is right there, i.e., the baseline for the change 
isn't obvious, this does not help the reader to
quickly interpret the importance of the change).}
In particular, we note that a foreground-free observation yields
$\lesssim5\%$-level errors on $D_A$.  
\pvm{Would be better to give actual numbers, i.e., this is a weirdly vague 
thing to say (I guess it means $\lesssim 5\%$ in all of the different redshift 
bins and that's why you don't just give 1 number?).}
While
...
\pvm{It seems from Fig. 8 that this result must not be *dramatically* affected 
by noise. What are some numbers with no foregrounds *and* no noise?}
